...
import io.harness.cdng.provision.azure.beans.AzureARMValidator;
import io.harness.cdng.provision.azure.beans.AzureARMValidator.AzureARMValidatorBuilder;
...
public class AzureARMRollbackStep extends TaskExecutableWithRollbackAndRbac<AzureARMTaskNGResponse> {
  ...
  public TaskRequest obtainTaskAfterRbac(
    ...
    if (azureARMRollbackStepParameters.getProvisionerIdentifier() == null) {
      return obtainSkipRollbackTask("Provisioner ID can't be null");
    }
    ...
    AzureARMValidator validator = validateIfRollbackCanBePerformed(templateDataOutput);
    if (!validator.isValid()) {
      return obtainSkipRollbackTask(validator.getErrorMsg());
    }
    ...
    AzureARMTaskNGParameters taskNGParameters = getAzureTaskNGParams(
        ambiance, stepParameters, (AzureConnectorDTO) connectorDTO.getConnectorConfig(), validator.getData());
  ...
  private AzureARMValidator validateIfRollbackCanBePerformed(AzureARMTemplateDataOutput output) {
    AzureARMValidatorBuilder builder = AzureARMValidator.builder();
    if (output == null) {
      builder.isValid(false);
      builder.errorMsg("There is no state saved for the provisioner ID");
    ...
    if (output.getResourceGroup() == null || output.getSubscriptionId() == null) {
      builder.isValid(false);
      builder.errorMsg(format("Missing relevant data for the rollback with resource group %s and subscription id %s",
          output.getResourceGroup(), output.getSubscriptionId()));
      return builder.build();
    }
    builder.isValid(true);
    builder.data(AzureARMPreDeploymentData.builder()
