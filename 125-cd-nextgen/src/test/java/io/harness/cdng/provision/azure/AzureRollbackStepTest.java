...
public class AzureRollbackStepTest extends CategoryTest {
  ...
  public void testObtainTaskWithNoProvisionerID() {
    ...
    assertThat(taskNGParameters.getTemplateBody()).isEqualTo("{foo}");
    assertThat(taskNGParameters.getParametersBody()).isEqualTo("{}");
  ...
  public void testValidateRollbackWithNoOutput() {
    ...
    assertThat(request.getSkipTaskRequest().getMessage()).isEqualTo("There is no state saved for the provisioner ID");
  }
  ...
  public void testValidateRollbackWithNoResourceGroupScope() {
    ...
    assertThat(request.getSkipTaskRequest().getMessage())
        .isEqualTo("The only scope allowed to do rollback is ResourceGroup. foobar is not supported");
  }
  ...
  public void testValidateRollbackWithNoResourceGroupID() {
    ...
    TaskRequest request = azureARMRollbackStep.obtainTaskAfterRbac(azureHelperTest.getAmbiance(), steps, inputPackage);
    assertThat(request.getSkipTaskRequest().getMessage())
        .isEqualTo("Missing relevant data for the rollback with resource group null and subscription id 123");
  }
  ...
  public void testValidateRollbackWithNoSubscriptionId() {
    ...
    TaskRequest request = azureARMRollbackStep.obtainTaskAfterRbac(azureHelperTest.getAmbiance(), steps, inputPackage);
    assertThat(request.getSkipTaskRequest().getMessage())
        .isEqualTo("Missing relevant data for the rollback with resource group 123 and subscription id null");
  }
