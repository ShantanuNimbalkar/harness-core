...
if [[ "" != "$MONGO_URI" ]]; then
  export MONGO_URI; yq -i '.mongo.uri=env(MONGO_URI)' $CONFIG_FILE
fi
...
if [[ "" != "$MONGO_LOCK_URI" ]]; then
  export MONGO_LOCK_URI; yq -i '.mongo.locksUri=env(MONGO_LOCK_URI)' $CONFIG_FILE
fi
...
if [[ "" != "$REDIS_SENTINELS" ]]; then
  ...
  for REDIS_SENTINEL_URL in "${REDIS_SENTINEL_URLS[@]}"; do
    ...
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.redisAtmosphereConfig.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.sentinelServersConfig.sentinelAddresses.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $REDISSON_CACHE_FILE
    INDEX=$(expr $INDEX + 1)
  done
fi
...
if [[ "" != "$EVENTS_FRAMEWORK_REDIS_SENTINELS" ]]; then
  ...
  for REDIS_SENTINEL_URL in "${SENTINEL_URLS[@]}"; do
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.eventsFramework.redis.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
    INDEX=$(expr $INDEX + 1)
  done
fi
