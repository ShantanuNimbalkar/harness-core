...
def manage_scheduler_jobs(event_json):
    for event in event_json:
        for inventory_type in INVENTORY_TYPE:
            update_parent_region_for_job("ce-gcp-%s-data-%s-%s" % (inventory_type, event["accountId"], event["connectorId"]))
...
def manage_inventory_data_scheduler_job(event, inventory_type):
    name = f"{parent}/jobs/ce-gcp-%s-data-%s-%s" % (inventory_type, event["accountId"], event["connectorId"])
