...
public class CloudWatchServiceImplTest extends CvNextGenTestBase {
  ...
  public void setup() throws IllegalAccessException {
    ...
    FieldUtils.writeField(cloudWatchService, "requestExecutor", requestExecutor, true);
  ...
  public void testFetchRegions() throws IOException {
    String responseObject = readResource("aws-regions.json");
    when(requestExecutor.execute(any())).thenReturn(JsonUtils.asMap(responseObject));
  ...
  public void testFetchSampleData() throws IOException {
    ...
    LinkedHashMap sampleDataResponse = cloudWatchService.fetchSampleData(builderFactory.getProjectParams(),
        connectorIdentifier, generateUuid(), generateUuid(), generateUuid(), generateUuid(), generateUuid());
