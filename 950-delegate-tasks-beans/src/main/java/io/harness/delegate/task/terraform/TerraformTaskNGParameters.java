...
public class TerraformTaskNGParameters
  ...
  public List<ExecutionCapability> fetchRequiredExecutionCapabilities(ExpressionEvaluator maskingEvaluator) {
      ...
      for (TerraformVarFileInfo varFileInfo : varFileInfos) {
        if (varFileInfo instanceof RemoteTerraformVarFileInfo) {
          ...
          if (gitFetchFilesConfig != null) {
            FileStoreFetchFilesConfig fileStoreFetchConfig = ((RemoteTerraformVarFileInfo) varFileInfo).getFilestoreFetchFilesConfig();
    ...
    if (backendConfigFileInfo != null) {
        if (backendConfigFileInfo instanceof RemoteTerraformBackendConfigFileInfo) {
          GitFetchFilesConfig gitFetchFilesConfig = ((RemoteTerraformBackendConfigFileInfo) backendConfigFileInfo).getGitFetchFilesConfig();
          FileStoreFetchFilesConfig fileStoreFetchConfig = ((RemoteTerraformBackendConfigFileInfo) backendConfigFileInfo).getFilestoreFetchFilesConfig();
          capabilities.addAll(getCapabilities(maskingEvaluator, gitFetchFilesConfig, fileStoreFetchConfig));
        }
    }
  ...
  private List<ExecutionCapability> getCapabilities(ExpressionEvaluator maskingEvaluator, GitFetchFilesConfig gitFetchFilesConfig,FileStoreFetchFilesConfig fileStoreFetchConfig) {
    List<ExecutionCapability> capabilities = new ArrayList<>();
    if (gitFetchFilesConfig != null) {
      capabilities.add(
